...
rod_eps = 0.50*np.ones((10, 10)) #Rod perm values
rho = PPC.Eps_to_Rho(epsr = rod_eps, plasma = True, w_src = w) #Initial Parameters

rho_opt, obj = PPC.Optimize_Waveguide(rho, 'src', 'prb', 0.010, 3, plasma = True)

## Save parameters and visualize ##############################################
PPC.Save_Params(rho_opt, 'params/10by10bentwaveguide_ez_w1_wp.csv')
print(PPC.Rho_to_Eps(rho = rho_opt, plasma = True, w_src = w))
PPC.Params_to_Exp(rho = rho_opt, src = 'src', plasma = True)
PPC.Viz_Sim_abs_opt(rho_opt, ['src'], 'plots/BentWaveguide_Ez_w1_wp.pdf', plasma = True)
PPC.Viz_Obj(obj, 'plots/BentWaveguide10by10_Ez_w1_wp_obj.pdf')
